...
def victory_screen(
    ...
    while frames:
        ...
        for sprite, player_coordinates in players_to_render:
            display.blit(sprite, player_coordinates)
        display.blit(text, (SCREEN_WIDTH / 2 - text.get_width() / 2, SCREEN_HEIGHT / 3))
        display.blit(final_sprite, (SCREEN_WIDTH / 2 - 32, SCREEN_HEIGHT / 2 - 32))
        pygame.display.update()
        frames -= 1
...
def main():
    ...
    p = menu(screen)
    winner, players_to_render = play_game(screen, p, hurt_frames)

    victory_screen(screen, winner, players_to_render, sit_frames.sprites[4])

    pygame.quit()


# idle_frames = SpriteSheet("idle.png")
# player_sprite = idle_frames.sprites[0]
"""
    # bowing = -1
    # bow_frames = 200
    ...
    # while bowing:
        # if bowing > 0:
        #     screen.blit(hurt_frames.sprites[::-1][bowing], (SCREEN_WIDTH/2 - 32, SCREEN_HEIGHT/2 - 32))
        #     bow_frames -= 1
        #     if not bow_frames:
        #         bowing -= 1
        #         bow_frames = 200
        # elif bowing < 0:
        #     screen.blit(frame_choice, (SCREEN_WIDTH/2 - 32, SCREEN_HEIGHT/2 - 32))
        # else:
        #     run = False
"""
